...
extension SCNetworkReachabilityFlags {
    ...
    var description: String {
        let onWWAN      = isOnWWANFlagSet ? "W" : "-"
        let onReachable = isReachableFlagSet ? "R" : "-"
        let onConnection = isConnectionRequiredFlagSet ? "c" : "-"
        let onTransient = isTransientConnectionFlagSet ? "t" : "-"
        let onIntervention = isInterventionRequiredFlagSet ? "i" : "-"
        let onConnectionOnTraffic = isConnectionOnTrafficFlagSet ? "C" : "-"
        let onConnectionOnDemand = isConnectionOnDemandFlagSet ? "D" : "-"
        let onLocalAddress = isLocalAddressFlagSet ? "l" : "-"
        let onDirect = isDirectFlagSet ? "d" : "-"

        return "\(onWWAN)\(onReachable) \(onConnection)\(onTransient)\(onIntervention)\(onConnectionOnTraffic)\(onConnectionOnDemand)\(onLocalAddress)\(onDirect)"
    }
}
